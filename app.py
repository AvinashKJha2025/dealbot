...
def load_deals_data():
    ...
    try:
        with open('data/deals_db.json', 'r') as file:
            return json.load(file)
    except FileNotFoundError:
...
def filter_deals_by_location(deals_data, location=None, pincode=None):
    ...
    for category, category_deals in deals_data.items():
        for deal in category_deals:
            ...
            if location and location.lower() in deal['deal_location'].lower():
                match_location = True
            if pincode and pincode in deal['deal_pincode']:
                match_pincode = True
...
def display_deals_page(deals, page, deals_per_page=4):
    ...
    for deal in page_deals:
        ...
        <div class="deal-card">
            ...
            <div class="deal-description">Vendor: {deal['deal_vendor']}</div>
            <div class="deal-details">Location: {deal['deal_location']} ({deal['deal_pincode']})</div>
            <div class="deal-details">Valid: {deal['start_date']} to {deal['expiry_date']}</div>
            <div class="deal-savings">Save: {deal['saving_amount']}</div>
            <div class="deal-code">Code: {deal['deal_code']}</div>
        </div>
        """, unsafe_allow_html=True)
...
def location_deals_page():
    ...
    if st.session_state.location_deals_state == 'input':
        ...
        st.markdown("Enter location (state) and/or pincode to find deals near you.")
        ...
        if st.button("🔍 Search Deals", key="search_deals"):
        ...
        if not deals:
            ...
            st.markdown(f"<span style='font-size:18px; color:#1f2937; font-weight:700;'>📍 Found {len(deals)} Deals</span>", unsafe_allow_html=True)
            ...
            with col2:
                st.markdown(f"<div class='page-info'>Page {current_page + 1} of {total_pages}</div>", unsafe_allow_html=True)
...
def chat_page():
    ...
    # Check if we're in location deals mode
